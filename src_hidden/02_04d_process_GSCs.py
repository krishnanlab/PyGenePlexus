...
for net_type in ['BioGRID', 'STRING-EXP', 'STRING', 'GIANT-TN']:
    net_genes = np.loadtxt('../data_backend2/Node_Orders/%s_nodelist.txt'%net_type,dtype=str)
    all_dis_genes = np.array([])
    universe_dis_genes = np.array([])
    good_dis_sets = {}
    with open('../data_hidden2/GSCs/propogated_annotations_doid.tsv','r') as f:
    ...
    np.savetxt('../data_backend2/GSCs/DisGeNet_%s_universe.txt'%net_type,universe_dis_genes,fmt='%s')
    with open('../data_backend2/GSCs/DisGeNet_%s_GoodSets.pickle'%net_type,'wb') as handle:
...
for net_type in ['BioGRID', 'STRING-EXP', 'STRING', 'GIANT-TN']:
    ...
    good_GO_sets = {}
    with open('../data_hidden2/GSCs/GO__propagated-annotations__Homo_sapiens__Entrez__BP__EXP_IDA_IPI_IMP_IGI_TAS_IC.tsv','r') as f:
        for idx, line in enumerate(f):
            if idx == 0:
                continue
            line = line.strip().split('\t')
    ...
    np.savetxt('../data_backend2/GSCs/GO_%s_universe.txt'%net_type,universe_GO_genes,fmt='%s')
    with open('../data_backend2/GSCs/GO_%s_GoodSets.pickle'%net_type,'wb') as handle:
