...
for net_type in ['BioGRID', 'STRING-EXP', 'STRING', 'GIANT-TN']:
    print(net_type)
    tic = time.time()
    nodelist = np.loadtxt('../data_backend/Node_Orders/%s_nodelist.txt'%net_type,dtype=str)
    ...
    with open('../data_backend/Edgelists/%s.edg'%net_type,'r') as f:
    ...
    np.save('../data_backend/Adjacency/%s_data.npy'%net_type,adj_mat)
    tic = time.time()
    ...
    np.save('../data_backend/Influence/%s_data.npy'%net_type,F_mat)

    tic = time.time()
    data = np.load('../data_backend/Adjacency/%s_data.npy'%net_type)
    ...
    tic = time.time()
    data = np.load('../data_backend/Influence/%s_data.npy'%net_type)
