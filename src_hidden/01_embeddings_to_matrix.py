...
for net_type in ['BioGRID', 'STRING-EXP', 'STRING', 'GIANT-TN']:
    ...
    with open('../data_hidden/embeddings/%s.emd'%net_type,'r') as f:
    ...
    np.savetxt('../data_backend/Node_Orders/%s_nodelist.txt'%net_type,nodelist,fmt='%s')
    np.save('../data_backend/Embedding/%s_data.npy'%net_type,data_matrix)
