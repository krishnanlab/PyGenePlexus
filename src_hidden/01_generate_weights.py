...
'''
In future add the positives to weight_dict as a ket called PosGenes
'''
...
with open('../data_backend/GSCs/%s_%s_GoodSets.pickle'%(gsc,net_type),'rb') as handle:
...
for idx in range(start_model,stop_model,1):
    ...
    pos_genes_in_net, genes_not_in_net, net_genes = utls.get_genes_in_network(pos_genes_tmp,net_type)
    negative_genes = utls.get_negatives(pos_genes_in_net,net_type,gsc)
    mdl_weights, probs, avgps = utls.run_SL(pos_genes_in_net,negative_genes,net_genes,net_type,features,False)
