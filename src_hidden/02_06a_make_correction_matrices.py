...
for anet in ['BioGRID','STRING-EXP','STRING','GIANT-TN']:
    with open('../data_backend2/GSCs/GO_%s_GoodSets.pickle'%anet,'rb') as handle:
        GO_sets = pickle.load(handle)
    with open('../data_backend2/GSCs/DisGeNet_%s_GoodSets.pickle'%anet,'rb') as handle:
        Dis_sets = pickle.load(handle)
    GO_order = list(GO_sets)
    Dis_order = list(Dis_sets)
    np.savetxt('../data_backend2/CorrectionMatrices/GO_%s_Orders.txt'%anet,GO_order,fmt='%s')
    np.savetxt('../data_backend2/CorrectionMatrices/DisGeNet_%s_Orders.txt'%anet,Dis_order,fmt='%s')
    for afeat in ['Embedding','Adjacency','Influence']:
        with open('../data_backend2/PreTrainedModels/GO_%s_%s_ModelWeights.pickle'%(anet,afeat),'rb') as handle:
            GO_weights = pickle.load(handle)
        with open('../data_backend2/PreTrainedModels/DisGeNet_%s_%s_ModelWeights.pickle'%(anet,afeat),'rb') as handle:
        ...
        for acombo in [('GO','DisGeNet'),('DisGeNet','DisGeNet'),('GO','GO'),('DisGeNet','GO')]:
            ...
            np.save('../data_backend2/CorrectionMatrices/%s_%s_%s_%s_CorMat.npy'%(gsc0,gsc1,anet,afeat),data)
